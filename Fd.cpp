...
distmesh::Functional distmesh::Fd::rectangular(
...
distmesh::Functional distmesh::Fd::rectangle(
    Eigen::Ref<Eigen::ArrayXXd const> const rectangle) {
    return DISTMESH_FUNCTIONAL({
...
distmesh::Functional distmesh::Fd::elliptical(
...
distmesh::Functional
distmesh::Fd::circular(double const radius,
    Eigen::Ref<Eigen::ArrayXd const> const midpoint) {
    return DISTMESH_FUNCTIONAL({
        if (midpoint.rows() == points.cols()) {
            return (points.rowwise() - midpoint.transpose())
...
distmesh::Functional distmesh::Fd::polygon(
